"""
Services — Orquestador de APIs de producción
1. MarketDataService   → Yahoo Finance + Alpaca WebSocket
2. BrokerService       → Alpaca Paper Trading (órdenes fraccionadas reales)
3. AMLService          → OpenSanctions + lógica OFAC local
4. CryptoService       → ECDSA P-256 firmas de órdenes
5. RoboAdvisorService  → Claude API con Prompts JSON estructurados
6. ReportService       → Excel multi-hoja + PDF ejecutivo ReportLab
7. CacheService        → Redis con TTL por tipo de dato
"""
import asyncio, json, time, random, hashlib, os
...
# ─────────────────────────────────────────────────────────
# 1. MARKET DATA — Yahoo Finance (sin API key, gratis)
# ─────────────────────────────────────────────────────────
...
def get_market_prices(tickers: list = None) -> dict:
    """
    Precios reales de Yahoo Finance. Cache interno de 15s.
    Fallback automático con precios realistas si Yahoo falla.
    """
    ...
    try:
        ...
        logger.warning(f"Yahoo Finance error: {e} — usando fallback")
        result = {t: _fallback_price(t) for t in tickers}
...
def _fallback_price(ticker: str) -> dict:
    """Precios simulados con movimiento realista (±0.5% por llamada)."""
...
def get_candles(ticker: str, period: str = "1mo", interval: str = "1d") -> list:
    """OHLCV para gráfico de velas japonesas."""
    try:
        import yfinance as yf
...
def _fallback_candles(ticker: str, period: str) -> list:
    ...
    for i in range(n):
        ...
        candles.append({"t": int(ts.timestamp() * 1000), "o": o, "h": h, "l": l, "c": c, "v": random.randint(10000, 500000)})
        price = c
    return candles

# ─────────────────────────────────────────────────────────
# 2. BROKER — Alpaca Paper Trading
# ─────────────────────────────────────────────────────────
...
def alpaca_get_account(key: str, secret: str) -> dict:
    if not key or key in ("DEMO_KEY", "demo"):
    ...
    try:
        r = requests.get(f"{ALPACA_BASE}/account", headers=_alpaca_headers(key, secret), timeout=8)
        return r.json() if r.ok else {"error": r.text, "status": r.status_code}
    except Exception as e:
        return {"error": str(e)}

def alpaca_place_order(key: str, secret: str, ticker: str, monto_usd: float, side: str = "buy",
                       tipo: str = "market", limit_price: float = None) -> dict:
    """
    Orden fraccionada real en Alpaca.
    notional = dólares → Alpaca calcula las fracciones automáticamente.
    """
    ...
    if not key or key in ("DEMO_KEY", "demo"):
        # Simulación paper fiel a Alpaca
...
def alpaca_get_positions(key: str, secret: str) -> list:
    if not key or key in ("DEMO_KEY", "demo"):
        return [
            ...
            {"symbol": "MSFT", "qty": "1.0543", "avg_entry_price": "410.00",
             "current_price": "432.10", "market_value": "455.55",
             "unrealized_pl": "23.30", "unrealized_plpc": "0.054"},
            {"symbol": "TSLA", "qty": "3.1200", "avg_entry_price": "220.00",
             "current_price": "248.30", "market_value": "774.70",
             "unrealized_pl": "88.30", "unrealized_plpc": "0.128"},
        ]
    try:
        r = requests.get(f"{ALPACA_BASE}/positions", headers=_alpaca_headers(key, secret), timeout=8)
...
# ─────────────────────────────────────────────────────────
# 3. AML — OpenSanctions + OFAC local
# ─────────────────────────────────────────────────────────
...
def aml_check_entidad(nombre: str, nit: str = None) -> dict:
    """
    Verifica entidad contra OpenSanctions + lista local OFAC.
    Retorna: status (clear/alert/blocked), score 0-1, detalle.
    """
    nombre_lower = nombre.lower()
    for bad in _LISTAS_NEGRAS_LOCAL:
        if bad in nombre_lower:
            return {"status": "blocked", "score": 1.0, "fuente": "local_ofac",
                    "detalle": f"Coincide con entidad sancionada localmente: '{bad}'"}
    try:
        payload = {"queries": {"q1": {"schema": "Company", "properties": {"name": [nombre]}}}}
        r = requests.post("https://api.opensanctions.org/match/default",
                          json=payload, headers={"Content-Type": "application/json"}, timeout=6)
        if r.ok:
            ...
            if results and results[0].get("score", 0) > 0.75:
                return {"status": "alert", "score": results[0]["score"],
                        ...
                        "detalle": f"Posible coincidencia OFAC/ONU: {results[0].get('caption', '')}"}
            return {"status": "clear", "score": 0, "fuente": "opensanctions",
                    "detalle": "Sin coincidencias en listas internacionales"}
    except Exception as e:
...
# ─────────────────────────────────────────────────────────
# 4. ECDSA — Importado desde security.py
# ─────────────────────────────────────────────────────────
from core.security import firmar_orden, verificar_firma, generar_nonce

# ─────────────────────────────────────────────────────────
# 5. ROBO-ADVISOR IA — Claude API + Prompts JSON estructurados
# ─────────────────────────────────────────────────────────
...
def robo_advisor_analizar(perfil_usuario: dict, portafolio: list, claude_api_key: str = "") -> dict:
    """
    Envía Prompt JSON a Claude y recibe análisis estructurado.
    Lógica determinística si no hay API key.
    """
    prompt_input = {
    ...
    if claude_api_key and claude_api_key not in ("", "demo"):
        try:
            ...
            if r.ok:
                ...
                resultado["_modelo"] = "claude-sonnet-4-6"
                return resultado
        except Exception as e:
            logger.warning(f"Claude API error: {e} — usando lógica local")

    # ── Lógica determinística local (sin Claude key) ──
    ...
    return {
        ...
        "acciones_recomendadas": (
            ...
            else ["QQQ - ETF Nasdaq 100", "NVDA - Semiconductores IA", "MSFT - Tecnología"] if perfil == "agresivo"
            else ["SPY - ETF S&P 500", "VTI - Mercado Total", "BND - Bonos 30%"]
        ),
        "explicacion_voz": f"Tu perfil es {perfil}. {'Alerta: concentración alta en ' + max_ticker + '.' if alerta else 'Portafolio equilibrado.'}",
        "_prompt_json_enviado": prompt_input,
        "_modelo": "local_deterministic"
    }

# ─────────────────────────────────────────────────────────
# 6. REPORTES — Excel real (openpyxl) + PDF (ReportLab)
# ─────────────────────────────────────────────────────────
def generar_excel_portafolio(usuario: dict, portafolio: list, ordenes: list,
    ...
    wb = Workbook()
    VRD = PatternFill("solid", fgColor="0D7A4E")   # verde InvestIQ
    AZL = PatternFill("solid", fgColor="0D3D7A")   # azul
    GRS = PatternFill("solid", fgColor="1E2835")   # gris oscuro
    ...
    def hoja_tabla(nombre, data, headers, fill):
        ...
        ws.sheet_properties.tabColor = "0D7A4E"
        ws.merge_cells(f"A1:{get_column_letter(len(headers))}1")
        ws["A1"] = f"InvestIQ — {nombre} — {datetime.now().strftime('%d/%m/%Y %H:%M')}"
        ws["A1"].font = TF; ws["A1"].alignment = Alignment(horizontal="center"); ws.row_dimensions[1].height = 28
    ...
    hoja_tabla("Órdenes", [
        [o.get("id"), o.get("ticker"), o.get("tipo","").upper(),
         ...
         round(o.get("precio_ejecucion",0),4), o.get("estado"), o.get("broker"),
         "✓" if o.get("firma_verificada") else "✗", o.get("creado","")]
        for o in ordenes
    ], ["#","Ticker","Tipo","Monto USD","Acciones","Precio Ejec.","Estado","Broker","Firma","Fecha"], AZL)
    ...
    filas = [("","",""),("Usuario",usuario.get("nombre","—"),""),
             ("Email",usuario.get("email","—"),""),("","",""),
             ("Valor total portafolio",f"$ {total_v:,.2f}",""),
             ("Ganancias / Pérdidas capital",f"$ {total_gp:,.2f}","✓" if total_gp>=0 else "⚠"),
             ("Dividendos recibidos",f"$ {total_div:,.4f}",""),
             ("Impuesto estimado (20%)",f"$ {max(total_gp,0)*0.20:,.2f}",""),
             ("","",""),("Generado",datetime.now().strftime("%d/%m/%Y %H:%M"),"")]
    for ri, (a,b,c) in enumerate(filas, 3):
        ws_res.cell(ri,1,a).font = Font(bold=True)
        ws_res.cell(ri,2,b); ws_res.cell(ri,3,c)
    for col in ws_res.columns:
        ws_res.column_dimensions[get_column_letter(col[0].column)].width = 30
...
def generar_pdf_reporte(usuario: dict, portafolio: list, metricas: dict, ruta: str) -> str:
    ...
    GRS = colors.HexColor("#1E2835"); LGRS = colors.HexColor("#F0F7F4")
    ...
    story.append(Paragraph(
        f'Generado: {datetime.now().strftime("%d de %B de %Y %H:%M")} · Usuario: {usuario.get("nombre","—")}',
    ...
    kpi_data = [
        ...
        ["Ganancia / Pérdida total", f"$ {metricas.get('ganancia_total',0):,.2f}",
         "✓" if metricas.get("ganancia_total",0) >= 0 else "⚠"],
        ["Posiciones activas", str(metricas.get("posiciones",0)), "✓"],
        ["Saldo disponible USD", f"$ {metricas.get('saldo',0):,.2f}", "✓"],
        ["Órdenes ejecutadas", str(metricas.get("ordenes",0)), "✓"],
    ]
    t = Table(kpi_data, colWidths=[7*cm, 6*cm, 3*cm])
    t.setStyle(TableStyle([
        ("BACKGROUND",(0,0),(-1,0),VRD),("TEXTCOLOR",(0,0),(-1,0),colors.white),
        ("FONTNAME",(0,0),(-1,0),"Helvetica-Bold"),("FONTSIZE",(0,0),(-1,-1),10),
        ("ROWBACKGROUNDS",(0,1),(-1,-1),[colors.white, LGRS]),
        ("GRID",(0,0),(-1,-1),0.4,colors.HexColor("#CCCCCC")),
        ("ALIGN",(1,1),(2,-1),"CENTER"),("TOPPADDING",(0,0),(-1,-1),7),("BOTTOMPADDING",(0,0),(-1,-1),7),
    ]))
    story.append(KeepTogether([Paragraph("<b>Indicadores Clave</b>", styles["Heading2"]), Spacer(1,0.2*cm), t]))
    ...
    story.append(Paragraph(
        f'<i>InvestIQ · Motor IA activo · Broker: Alpaca Paper Trading · {datetime.now().strftime("%Y")}</i>',
        ParagraphStyle("F", fontSize=8, textColor=colors.grey, alignment=TA_CENTER)))
    doc.build(story)
    return ruta

# ─────────────────────────────────────────────────────────
# 7. REDIS CACHE — con fallback en memoria si Redis no está
# ─────────────────────────────────────────────────────────
_mem_cache: dict = {}
