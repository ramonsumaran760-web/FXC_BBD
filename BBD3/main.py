...
from core.rate_limit import limiter, get_limit, rate_limit_handler
from core.monitoring import init_sentry, capturar_error, alerta_broker_error, alerta_aml_blocked
...
async def candles(request: Request, ticker: str, period: str = "1mo", interval: str = "1d"):
...
async def robo_advisor(request: Request, data: RoboAdvisorSchema, db: AsyncSession = Depends(get_db)):
    user_res = await db.execute(select(Usuario).where(Usuario.id == 1))
    user = user_res.scalar_one()
...
async def exportar_excel(request: Request, db: AsyncSession = Depends(get_db)):
    user_res = await db.execute(select(Usuario).where(Usuario.id == 1))
    user = user_res.scalar_one()
...
async def exportar_pdf(request: Request, db: AsyncSession = Depends(get_db)):
    user_res = await db.execute(select(Usuario).where(Usuario.id == 1))
    user = user_res.scalar_one()
    pos_res = await db.execute(select(PosicionPortafolio).where(
        PosicionPortafolio.usuario_id == 1, PosicionPortafolio.acciones > 0))
...
async def health():
    ...
    try:
        ...
        cache_set("health_check", {"ok": True}, ttl=5)
        redis_ok = cache_get("health_check") is not None
    except Exception:
        pass
    return {
        "status": "ok",
        ...
        "services": {
            "database": "ok",
            "broker": "alpaca_paper",
            "ia": "claude_ready" if os.getenv("CLAUDE_API_KEY") else "local_mode",
            "redis": "ok" if redis_ok else "memory_fallback",
            "sentry": "active" if sentry_ok else "not_configured",
            "stripe": "active" if os.getenv("STRIPE_SECRET_KEY") else "demo_mode",
            "mercadopago": "active" if os.getenv("MERCADOPAGO_ACCESS_TOKEN") else "demo_mode",
        }
    }
